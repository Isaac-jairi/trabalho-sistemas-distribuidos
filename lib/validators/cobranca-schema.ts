...
export const cobrancaFormSchema = z.object({
  ...
  nomeSacadoCobranca: z.string().min(3, { message: 'Nome deve ter pelo menos 3 caracteres' }),
  numeroInscricaoSacadoCobranca: z.string().min(11, { message: 'Número de inscrição inválido' }),
  textoEnderecoSacadoCobranca: z.string().min(5, { message: 'Endereço deve ter pelo menos 5 caracteres' }),
  nomeBairroSacadoCobranca: z.string().min(2, { message: 'Bairro deve ter pelo menos 2 caracteres' }),
  nomeMunicipioSacadoCobranca: z.string().min(2, { message: 'Município deve ter pelo menos 2 caracteres' }),
  siglaUnidadeFederacaoSacadoCobranca: z.string().length(2, { message: 'UF deve ter 2 caracteres' }),
  numeroCepSacadoCobranca: z.string()
    .min(8, { message: 'CEP inválido' })
    .refine(isCepValid, { message: 'Formato de CEP inválido' }),
  
  // Title/Invoice Information
  valorDescontoTitulo: z.coerce.number()
    .nonnegative({ message: 'O valor de desconto não pode ser negativo' }),
  valorJuroMoraTitulo: z.coerce.number()
    .nonnegative({ message: 'O valor de juros não pode ser negativo' }),
  numeroTituloCedenteCobranca: z.string().min(1, { message: 'Número do título é obrigatório' }),
